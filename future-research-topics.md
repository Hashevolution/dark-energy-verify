...
| **D** 차별 카드(존재론) | 이론 | **〔§9 수행 완료〕** §9.8 부호변화 = **측도 아티팩트 확정**(CKW 통일측도서 τ3≥0, 부호변화 불가) | **닫힘 (아티팩트, 카드 소진)** |
...
## 9. 〔실행 기록 2026-06-19〕 과제 D 수행 결과 → 측도 아티팩트 확정, 카드 닫힘

**실행:** `experiments/phaseD_monogamy_excess.py`, `phaseD_selffalsify.py`.
상세: `experiments/phaseD_RESULTS.md`.

**대상:** 노트 §9.8 "얽힘 종류=독립 실재 층"(레포 유일 독창 카드)의 잔여 신호 — 전역얽힘
E(폰노이만) − CKW잔차 τ_res(tangle) 초과분의 +0.56→−0.31 **부호변화**.

**결판:** GHZ↔W 순수 3-큐비트 족(=§9.8 종류 분리)에서 CKW가 정확 등식.
- MISMATCH(엔트로피−tangle)는 부호변화 **재현**(+1.0→−0.415).
- UNIFIED(통일 tangle, =τ3)는 **정확히 ≥0, 부호변화 없음**. CKW 항등식 기계정밀도(1e-8).
- 자가반증: 부호변화가 전역측도·잔차측도·상태족 *전부에* 의존 = 규약 산물. τ3만 불변.

**판정:** **과제 D = 닫힘. §9.8 부호변화 = 측도 아티팩트**(노트의 자기 의심을 엄밀 확정).
종류 분리 = 표준 CKW monogamy 재현, 초과 신호 없음.

**함의:** 레포의 **유일 독창 이론 카드 소진.** 손실이 아니라 *지도 완성*(핸드오버 §3.D의
"결판" 지시 이행). → 이론 단독 차별 입구 닫힘 → 원 전략("데이터(b)서 출발") 재유효.

**남은 좌표:** E′(큰 홀로그래픽 코드, 무거움) · C′(데이터 통계/계통 분해, 독립 실행 가능).
→ **다음 권고: C′**(가볍고 독립적이며, 이론 카드 소진 후 데이터 트랙이 유일한 차별 입구).
